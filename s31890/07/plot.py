...
def plot_max_score_by_dense_layers(trials_df):
    plt.figure(figsize=(10, 6))
    sns.barplot(
        data=trials_df,
        x="num_dense_layers",
    ...
    plt.title("Maximum Score by Number of Dense Layers")
    plt.xlabel("Number of Dense Layers")
    plt.ylabel("Maximum Score")
    plt.grid(axis="y", linestyle="--", alpha=0.7)
    plt.tight_layout()
    plt.savefig("plot_max_score_by_dense_layers.png")
...
def visualize_all_trials(trials_data):
    ...
    print("running: Bar Chart (Max Score by Dense Layers)...")
    plot_max_score_by_dense_layers(trials_df)
